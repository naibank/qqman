...
manhattan <- function(x, chr="CHR", bp="BP", p="P", snp="SNP", 
                      ...
                      highlight=NULL, logp=TRUE, annotatePval = NULL, annotateTop = TRUE, cex = 0.7, y.off = 0.5, x.off = 0, ...) {
        ...
        if (annotateTop == FALSE) {
          if (logp) {
              with(subset(d, P <= annotatePval), 
                   textxy(pos, -log10(P)+y.off, offset = x.off, labs = topHits$SNP, cex = cex), ...)
          } else
              with(subset(d, P >= annotatePval), 
                   textxy(pos, P+y.off, offset = x.off, labs = topHits$SNP, cex = cex), ...)
        }
        else {
            ...
            if (logp ){
                textxy(topSNPs$pos, -log10(topSNPs$P)+y.off, offset = x.off, labs = topSNPs$SNP, cex = cex, ...)
            } else
              textxy(topSNPs$pos, topSNPs$P+y.off, offset = x.off, labs = topSNPs$SNP, cex = cex, ...)
        }
    }  
    par(xpd = FALSE)
}
